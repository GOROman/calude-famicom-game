...
:
    ...
    jsr write_column
@no_col:
    ; ---- タイトル: BG パレットを毎フレーム書く (fade_amt で暗転量を制御) ----
    lda game_state
    cmp #4
    bne @no_fade
    bit PPUSTATUS
...
@wc2:
    ...
    jmp @no_split
@cleared:
    lda #51             ; 目の帯 (y≈50-83) だけバンク2 = 閉じ目/白目タイル
    jsr delay_lines
    lda #2
    jsr set_chr_bank
    lda #33
...
@hit:
    ...
    sta PPUCTRL
    lda #93             ; y=136 → 228 まで待って最下行をバンク2に
    jsr delay_lines     ; (ビルド情報のミニフォント)
    lda #2
    jsr set_chr_bank
@no_split:
@skip:
